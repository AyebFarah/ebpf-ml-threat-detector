...
from observation.features.utils.ip_utils import is_private_ip, is_external_ip
from observation.features.utils.string_features import domain_label_stats
...
def basic_counts(rows, ssh_total, extra_flows=None, extra_dns_events=None) -> dict:
    n = len(rows)
    flow_count = n + (len(extra_flows) if extra_flows else 0)
    if extra_dns_events:
        dns_queries = sum(1 for e in extra_dns_events if e.get("dns_event_type") == "dns_query")
    else:
        dns_queries = sum(1 for r in rows if r["dns_domain"])
    return {
        "event_count": n,
        "tcp_connect_count": n,
...
def dns_features(rows, extra_events=None) -> dict:
    ...
    if extra_events:
        # Raw capture already holds every query and response, including those the
        # correlator attached to a connection, so use it alone and nothing is counted twice.
        seen, domains, resolved_ips, nxdomain = set(), [], set(), 0
        for e in extra_events:
            ...
            if key:
                if key in seen:
                    continue
                seen.add(key)
            kind = e.get("dns_event_type")
            if kind == "dns_query" and e.get("dns_domain"):
